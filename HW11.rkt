...
(define resolve
  (lambda (varName env)
    (cond
      ((null? env) #f)
      ((null? (car env)) (resolve varName (cdr env)))
...
(define construct-scope
  (lambda (list-of-2-lists env)
    (if (null? list-of-2-lists) '()
        (let* ((two-list (construct-scope-helper (car list-of-2-lists) env)))
          ;(display (cdr list-of-2-lists))))))
          (cons two-list (construct-scope (cdr list-of-2-lists) (extend-env (list two-list) env)))))))
       ;(cons (list (car (car list-of-2-lists)) (run-parsed-code (cadr (car list-of-2-lists)) env)) (construct-scope (cdr list-of-2-lists) (extend-env (list (car list-of-2-lists)) env)))))))
  ;  (map (lambda (2-list) (list (car 2-list) (run-parsed-code (cadr 2-list) env))) list-of-2-lists)))

(define construct-scope-helper
  (lambda (2-list env)
    (list (car 2-list) (run-parsed-code (cadr 2-list) env))))
...
(define env-test
  (lambda (let-env env)
    (cond
      ((null? let-env) env)
      (else
       (cons (car let-env) (env-test (cdr let-env) env))))))
...
(define sample-no-code '(call (function () (local-vars ((a 27) (b a)) b))))
